...
from train_results_resnetBaseline import FitResult, BatchResult, EpochResult
        ...
        for epoch in range(num_epochs):
            ...
            if run != None:
                run.log({
                    ...
                    # Add Train Agreement on Wrong student samples - TODO
                    # Add Train Agreement on Wrong teacher samples - TODO
                    # Add Train argument that represent the distance on the tail - TODO
                    "Test Loss": sum(test_result.losses)/len(test_result.losses),
                    "Test Accuracy": test_result.accuracy,
                    # Add Test Agreement on Wrong student samples - TODO
                    # Add Test Agreement on Wrong teacher samples - TODO
                    # Add Test argument that represent the distance on the tail - TODO
                    })
            # ========================
...
class ClassifierTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        class_score = self.model.forward(X)
        batch_loss = self.loss_fn.forward(class_score, y)
    ...
    def test_batch(self, batch) -> BatchResult:
        ...
        with torch.no_grad():
            ...
            class_score = self.model.forward(X)
            batch_loss = self.loss_fn.forward(class_score, y).item()
            # num_correct = torch.sum(y == torch.argmax(class_score, dim=1)).item()
            predictions = self.model.classify(X)
            num_correct = (predictions == y).float().sum().item()
            # ========================
...
class LayerTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        self.optimizer.step()
        num_correct = (torch.argmax(values, dim=1) == y).sum()
        # ========================
